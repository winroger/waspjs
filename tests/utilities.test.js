...
describe('collider bounds checks', () => {
  function makeMesh(offsetX = 0) {
  ...
  it('detects overlapping meshes in checkCollisionsWithParts', () => {
    const collider = new Collider([makeMesh(0)], false, false, [], []);
    const parts = [{ collider: new Collider([makeMesh(0.25)], false, false, [], []) }];
    expect(collider.checkCollisionsWithParts(parts)).toBe(true);
  });

  it('does not report collision for separated meshes in checkCollisionsById', () => {
    const collider = new Collider([makeMesh(0)], false, false, [], []);
    const parts = [{ collider: new Collider([makeMesh(5)], false, false, [], []) }];
    expect(collider.checkCollisionsById(parts, [0])).toBe(false);
  });
});
